...
"""
Для выполнения я реализовала классы лист и внутренний узел. Получилось, что для каждого класса существует только
один метод, что насколько я знаю не очень хорошо. Возможно, лучшим решением было бы сделать функции, сразу наполняющие
итоговый словарь. Но реализация через классы для меня более наглядная. Также для упрощения кода я использовала
модуль heapq для создания очереди с приоритетами.
